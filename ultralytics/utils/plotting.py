...
def plot_labels(boxes, cls, names=(), save_dir=Path(""), on_plot=None):
    ...
    x = boxes[:, :4] if len(boxes) else np.zeros((0, 4), dtype=np.float32)
    ...
    ax[2].hist2d(x[:, 0], x[:, 1], bins=50, cmap=subplot_3_4_color)
    ax[2].set_xlabel("x")
    ax[2].set_ylabel("y")
    ax[3].hist2d(x[:, 2], x[:, 3], bins=50, cmap=subplot_3_4_color)
    ...
    for i, f in enumerate(files):
        try:
            try:
                ...
                data = pl.read_csv(f, infer_schema_length=None).to_dict(as_series=False)
            except Exception:
                import pandas as pd

                data = pd.read_csv(f).to_dict(orient="list")
            if i == 0:
                for c in data.keys():
                    if "loss" in c:
                        loss_keys.append(c)
                ...
                ax = ax.ravel()
            x = np.asarray(data[next(iter(data))], dtype=float)
            for i, j in enumerate(columns):
                y = np.asarray(data[j], dtype=float)
